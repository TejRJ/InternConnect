...
export default function RSidebar({ isOpen, onClose }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    if ((session as Session)?.user?.role === "recruiter") loadCount();
  }, [session]);
  ...
  useEffect(() => {
    ...
    if ((session as Session)?.user?.role === "recruiter") loadApps();
  }, [session]);
    ...
    <>
      ...
      >
          ...
          <ul className="space-y-1">
            <li>
              <a
                ...
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium hover:bg-indigo-50 hover:text-indigo-700 ${
                  pathname === "/r" ? "text-indigo-700 bg-indigo-50 ring-1 ring-indigo-100" : "text-gray-700"
                }`}
              >
            ...
            <li>
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  router.push("/ropportunities");
                  onClose();
                }}
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium hover:bg-indigo-50 hover:text-indigo-700 ${
                  pathname.startsWith("/ropportunities") ? "text-indigo-700 bg-indigo-50 ring-1 ring-indigo-100" : "text-gray-700"
                }`}
              >
            ...
            <li>
              <a
                ...
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium hover:bg-indigo-50 hover:text-indigo-700 ${
                  pathname.startsWith("/rlistings") ? "text-indigo-700 bg-indigo-50 ring-1 ring-indigo-100" : "text-gray-700"
                }`}
              >
            ...
            <li>
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  router.push("/rapplications");
                  onClose();
                }}
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium hover:bg-indigo-50 hover:text-indigo-700 ${
                  pathname.startsWith("/rapplications") ? "text-indigo-700 bg-indigo-50 ring-1 ring-indigo-100" : "text-gray-700"
                }`}
              >
          ...
          <div className="mt-6">
            ...
            <div className="space-y-3 px-2">
              ...
              <div className="flex items-center justify-between rounded-lg bg-slate-50 ring-1 ring-slate-200 px-3 py-2">
                ...
                <span className="text-sm font-semibold rounded-full bg-emerald-100 text-emerald-700 px-2 py-0.5">
                  {appsCount}
                </span>
              </div>
            </div>
        ...
        <div className="mt-auto px-6 py-4 ring-1 ring-slate-200 absolute bottom-0 left-0 right-0 bg-white/80 backdrop-blur">
          ...
          <div className="mt-3">
            <button onClick={() => { signOut({ callbackUrl: "/auth/login" }); }} className="w-full rounded-full bg-slate-900 px-3 py-1 text-xs font-medium text-white shadow hover:bg-black cursor-pointer transition-colors">Sign out</button>
          </div>
        </div>
      </aside>
    </>
